...
class Dataset(object):
    def __init__(self, dataset_name=None, go_backwards=False, pad_mode=PadMode.POST):
        ...
        self.attribute_keys = None # 事件的属性** 原始的事件日志只有name, Activity两个属性
        self.classes = None
        self.labels = None
        self.encoders = None
        ...
        if self.dataset_name is not None:
            print('1 Dataset-init:self.dataset_name=', self.dataset_name)
    ...
    def load(self, dataset_name):
        ...
        :return:
        """
        print('2 Dataset-load:self.dataset_name=', dataset_name)
        el_file = EventLogFile(dataset_name) # 若缓存中有事件日志，则从缓存中获取
        self.dataset_name = el_file.name
        print('4 Dataset-load:self.self.dataset_name=', self.dataset_name)

        # Check for cache
        if el_file.cache_file.exists():
            print('6 el_file.cache_file.exists():el_file.path=', el_file.path)
            self._load_dataset_from_cache(el_file.cache_file)

        # Else generate from event log
        elif el_file.path.exists():
            print('6 el_file.path.exists:el_file.path=', el_file.path)
    ...
    def _cache_dataset(self, file):
        with gzip.open(file, 'wb') as f:
            pickle.dump((self._features, self.classes, self.labels, self._case_lens, self._attribute_dims,
                         self.encoders, self.attribute_types, self.attribute_keys), f, protocol=4)

    @property
    def mask(self):
        print('april-dataset-mask')
        if self._mask is None:
            print("april-dataset-mask self._mask is None")
            self._mask = np.ones(self._features[0].shape, dtype=bool)
            for m, j in zip(self._mask, self.case_lens):
                m[:j] = False
        print('april-dataset-mask self.pad_mode=', self.pad_mode)
    ...
    def binary_targets(self):
        ...
        print("april-dataset-binary_targets")
    ...
    def flat_features(self):
        ...
        :return:
        """
        print("april-dataset-flat_features")
    ...
    def remove_time_dimension(x):
        print('14 april-dataset-remove_time_dimension  x.shape[0]=', x.shape[0], ',np.product(x.shape[1:])=', np.product(x.shape[1:]))
    ...
    def flat_features_2d(self):
        ...
        :return:
        """
        print('13 april-dataset-flat_features_2d')
    ...
    def _get_classes_and_labels_from_event_log(event_log):
        ...
        :return:
        """
        print('10 april-dataset-EventLog-_get_classes_and_labels_from_event_log')
        ...
        print('10 type(targets)=', type(targets), ', type(labels)=', type(labels))
        print('10 targets.shape=', targets.shape, ', labels.shape=', labels.shape)
        print('10 targets[0]=', targets[0], ', labels[0]=', labels[0])
    ...
    def _from_event_log(event_log, include_attributes=None):
        ...
        """
        print('9 april-dataset-EventLog-_from_event_log')
        print('include_attributes is None=', include_attributes is None)
        if include_attributes is None:
            include_attributes = event_log.event_attribute_keys

        feature_columns = dict(name=[])
        case_lens = []
        print('april-dataset-_from_event_log:include_attributes=', include_attributes)
        ...
        print('9 april-dataset-EventLog-_from_event_log:case_lens=', case_lens)
        return features, case_lens, attr_types, encoders

    def from_event_log(self, event_log):
        print('8 april-dataset-EventLog-from_event_log')
        """
        ...
        print("april-dataset-(from_event_log()) self.attribute_keys=", self.attribute_keys)
