...
#         elif len(self.activities) == 1 and self.activities[0] == '':
#             event = Event(name=f'Random activity')
#         else:
...
class ReworkAnomaly(Anomaly):
    ...
    def __init__(self, max_sequence_size=1, max_distance=0):
...
#         size = np.random.randint(2, min(len(case), self.max_sequence_size) + 1)
        size = np.random.randint(1, min(len(case), self.max_sequence_size) + 1)
#         start = np.random.randint(0, len(case) - size + 1)
        start = np.random.randint(1, len(case) - size + 1) # 默认第一个活动（下标为0）是正常的，从第二个活动（下标为1）开始注入异常 2021.08.22
...
class LateAnomaly(Anomaly):
    ...
    def apply_to_case(self, case):
        if len(case) <= 3:  # if len(case) <= 2:
            return NoneAnomaly().apply_to_case(case)

        size = np.random.randint(1, min(len(case) - 1, self.max_sequence_size + 1))
        distance = np.random.randint(1, min(len(case) - size, self.max_distance + 1))
        s = np.random.randint(1, len(case) - size - distance + 1) # 2021.08.22
        i = s + distance
...
class EarlyAnomaly(Anomaly):
    ...
    def apply_to_case(self, case):
        ...
        size = np.random.randint(1, min(len(case) - 1, self.max_sequence_size + 1))
        distance = np.random.randint(1, min(len(case) - size, self.max_distance + 1))
        s = np.random.randint(distance+1, len(case) - size)
#         s = np.random.randint(distance+1, len(case)) # distance + 1 ，表示从第二个活动才有可能发生early异常 2021.08.22
        i = s - distance
...
#                 shift_from=int(s + size),
                shift_from=int(s + size - 1), # 2021.08.22
                shift_to=int(i),
                size=int(size)
            )
        )
...
#         places = sorted(np.random.choice(np.arange(len(case) - 1, step=2), num_replacements, replace=False))
        places = sorted(np.random.choice(np.arange(1, len(case) - 1, step=1), num_replacements, replace=False)) # 2021.08.22
        t = case.events
...
#         insert_places = sorted(np.random.choice(range(1, len(case) - 1), num_inserts, replace=False))
        insert_places = sorted(np.random.choice(range(1, len(case)+1), num_inserts, replace=False)) # 2021.08.22
...
class SkipAnomaly(Anomaly):
    ...
    def __init__(self, max_skips=1): # max_skips=2 -> max_skips=1  # 2021.08.22
...
#         num_skips = np.random.randint(2, min(int(len(case) / 2), self.max_skips) + 1)
        num_skips = np.random.randint(1, min(int(len(case) / 2), self.max_skips) + 1) # 2021.08.22
#         skip_places = sorted(np.random.choice(range(len(case) - 1), num_skips, replace=False))
        skip_places = sorted(np.random.choice(range(1, len(case) - 1), num_skips, replace=False))  # 2021.08.22
