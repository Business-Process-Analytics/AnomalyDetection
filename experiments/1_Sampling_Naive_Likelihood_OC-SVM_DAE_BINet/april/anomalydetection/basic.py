...
class OneClassSVM(AnomalyDetector):
    ...
    def __init__(self, model=None, nu=.5):
        ...
        print('11 class OneClassSVM(AnomalyDetector)')
        self.nu = nu

    def fit(self, dataset):
        print('12 OneClassSVM fit()')
        features = dataset.flat_features_2d
        print('12 OneClassSVM fit() features.shape=', features.shape)
        from sklearn.svm import OneClassSVM
        self._model = OneClassSVM(nu=self.nu)
        self._model.fit(features)

    def detect(self, dataset):
        print('++++++++++OneClassSVM detect()')
        features = dataset.flat_features_2d
        ...
        print('+++++++++++++++++april-anomaly-basic-OneClassSVM detect() features.shape=', features.shape)
