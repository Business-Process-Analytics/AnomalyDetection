...
class EventLog(object):
    ...
    def event_attribute_keys(self):
        attributes = ['name']
#         print('self.attributes.keys()=', self.attributes.keys())
#         ['org:group', 'resource country', 'organization country','org:resource','org:role']
        if 'global_attributes' in self.attributes.keys() and 'event' in self.attributes['global_attributes'].keys():
#             print('event_attribute_keys if')
            ignored = ['concept:name', 'time:timestamp', 'lifecycle:transition', 'EventID', 'activityNameEN',
                       ...
                       'organization involved', 'impact', 'org:group', 'resource country', 
                       'organization country','org:resource','org:role']

            attributes += sorted(
                [key for key in self.attributes['global_attributes']['event'].keys() if key not in ignored])
            
        else:
            print('event_attribute_keys else')
            ignored = ['concept:name', 'time:timestamp', 'lifecycle:transition', 'EventID', 'activityNameEN',
                       'activityNameNL', 'dateFinished', 'question', 'product', 'EventOrigin', 'Action',
                       'organization involved', 'impact', 'org:group', 'resource country', 
                       'organization country','org:resource','org:role']
            ...
            attributes += sorted(
                [key for key in list(self.cases[0].events[0].attributes.keys()) if key not in ignored ])
        return attributes
    ...
    def get_attribute_types(self, attributes=None):
        ...
        for a in attributes:
            print('april-processmining-log: a=', a)
            if a == 'name':
    ...
    def load(eventlog_name):
        ...
        :return:
        """
        print('7 april-process-log-EventLog-load:eventlog_name=', eventlog_name)
    ...
    def from_xes(file_path):
        ...
        """
#         <classifier name="Event Name" scope="event" keys="concept:name"/>
...
#             print('attr=', attr, ', type(attr)=', type(attr))
            if 'id' in attr: # 删除事件属性中的id，避免与case_id冲突
                attr.pop('id')
#             print('**************attr=', attr)
            case_id = None
...
#             name = attr['concept:name']
# #             if 'W_' in name and attr['lifecycle:transition'] == 'COMPLETE':
#                 #if 'A_' in name:
#             if 'O_' in name: # for process dataset BPIC2012_O
#                 timestamp = None
...
#     #                 name = '+'.join(values) # 活动的名称 注掉：2021.09.04
#                 print('before, april-processmining-parse_event(): ', type(attr), ', attr=', attr)
#                 attr_pop_list = ['org:group', 'resource country', 'organization country','org:resource', 'organization involved', 'org:role','impact', 'product']
#                 for index in range(len(attr_pop_list)):
#                     if attr_pop_list[index] in attr:
#                         attr.pop(attr_pop_list[index])
#                 attr['Activity'] = name
#                 print('after, april-processmining-parse_event(): ', type(attr), ', attr=', attr)
#                 print('-----------------------------------------------')
#                 return Event(name=name, timestamp=timestamp, **attr)
        ...
        def parse_event(event):
            ...
            name = ''
#             print('processmining-log: len(classifiers)=', len(classifiers))
            if len(classifiers) > 0:
                keys = classifiers[0]['keys']
                check_keys = [key for key in keys if key not in attr]
                if len(check_keys) > 0:
